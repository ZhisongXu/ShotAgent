...
Both temporal metrics exclude detected shot boundaries. Warping error is still
...
Do not rank cross-content target/reference pairs using raw RGB/Lab histogram
distance, histogram correlation, output-to-reference SSIM/LPIPS/Delta-E, or
changed-pixel fraction. Those quantities mostly measure which colors and
objects happen to occur in the two scenes. BRISQUE and NIQE may be included in
an appendix but are not reliable arbiters of intentional cinematic looks.
...
The report schema is `reference-video-grade-benchmark/v2-no-gt`. It explicitly
