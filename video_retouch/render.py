...
def render_grade_frames(
    ...
    *,
    ...
    if parameters.shape != (len(frames), 12):
        raise ValueError(
            "frame_parameters must have shape [number_of_frames, 12]."
        )
    if not frames:
        return
    size = frames[0].size
    ...
    with torch.inference_mode():
        for start in range(0, len(frames), batch_size):
            ...
            arrays = np.stack(
                [np.asarray(frame.convert("RGB"), dtype=np.float32) for frame in batch_frames]
            ) / 255.0
            images = (
            ...
            values = torch.from_numpy(
                parameters[start : start + len(batch_frames)]
            ).to(selected_device, non_blocking=True)
